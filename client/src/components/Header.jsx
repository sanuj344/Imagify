...
const Header = () => {
    ...
    return (
        <motion.div initial={{ opacity: 0.2, y: 100 }} transition={{ duration: 1 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className='flex flex-col justify-center items-center text-center my-20'>
            ...
            initial={{opacity:0}}
            animate={{opacity:1}}
            transition={{delay:0.6,duration:2}} className='text-center max-w-xl mx-auto mt-5'>Unleash your creativity with AI. Turn your imagination into visual art in seconds - just tupe, and watch the magic happen.</motion.p>
            <motion.button
            ...
            className='flex flex-wrap gap-3 mt-16'>
                {Array(6).fill('').map((item, index) => (
                    ...
                    max-sm:w-10' src={index % 2 ? assets.sample_img_1 : assets.sample_img_2} alt=""
                        key={index} width={70} />
                ))}
            </motion.div>
            <motion.p
